...
class _BgBorrowReportPageState extends State<BgBorrowReportPage> {
  ...
  // ปุ่มสลับกรองสถานะการยืม (ทั้งหมด / วันนี้ / ยังไม่คืน)
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        Widget tableContent = _isLoading
            ? const Center(child: CircularProgressIndicator())
            : _borrowReportList.isEmpty
                ? const Center(
                    child: Padding(
                      padding: EdgeInsets.all(32.0),
                      child: Text('ไม่พบข้อมูลการยืม'),
                    ),
                  )
                : Scrollbar(
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              // สลับใช้ Expanded / Flexible ตามชนิดของพื้นที่แม่
              if (hasBoundedHeight)
                Expanded(child: tableContent)
              else
                Flexible(child: tableContent),
            ],
          ),
        );
      },
